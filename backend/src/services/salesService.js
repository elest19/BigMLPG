...
export async function listSales({
  ...
  `SELECT
      ...
      ch.payment_option,
      COALESCE(initial_credit.balance_paid, 0)::numeric AS initial_credit_balance_paid,
      GREATEST(sr.total_amount - COALESCE(pay.total_paid, 0), 0)::numeric AS remaining_balance,
  ...
  const combinedRows = [...salesDataResult.rows, ...paymentDataResult.rows]
    ...
    .map((row) => ({
      ...
      balance_paid: row.entry_type === 'payment'
        ? Number(row.balance_paid || 0)
        : getSalesLogBalancePaid({
            paymentOption: row.payment_option,
            initialCreditBalancePaid: row.initial_credit_balance_paid,
            fallbackBalancePaid: row.balance_paid,
          }),
...
export async function createSale(payload) {
  ...
  try {
    ...
    if (
      ...
      payload.location.trim() !== (customer.location || "").trim()
    ) {
      await customerService.updateCustomer(customer.customer_id, {
        name: customer.name,
        location: payload.location,
...
export async function deleteAllSales() {
  ...
  try {
    ...
    const existingSalesResult = await client.query(
      `SELECT sale_id, product_id, sale_quantity, lpg_tank_variant
       FROM sales_records`,
    );

    for (const existing of existingSalesResult.rows) {
      await reverseSaleStockEffect(existing, client);
    }

    await client.query(`DELETE FROM credit_history WHERE sales_id IN (SELECT sale_id FROM sales_records)`);
    const deleteResult = await client.query(`DELETE FROM sales_records`);
