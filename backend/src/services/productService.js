...
export async function listProducts({
  ...
  const visibleProducts = result.rows
    .filter((product) => matchesStockTier(product, stockTier))
    .sort((left, right) => {
      ...
      const createdCompare = new Date(left.created_at || 0) - new Date(right.created_at || 0);
