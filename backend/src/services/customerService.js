import { query } from '../config/db.js';
import { AppError } from '../middleware/errorHandler.js';

export async function listCustomers(search = '') {
  const result = await query(
    `SELECT customer_id, name, location, phone_number, created_at
     FROM customers
     WHERE ($1 = '' OR name ILIKE $2 OR location ILIKE $2 OR phone_number ILIKE $2)
     ORDER BY name ASC`,
    [search, `%${search}%`]
  );
  return result.rows;
}

export async function getCustomerById(customerId) {
  const result = await query(
    `SELECT customer_id, name, location, phone_number, created_at
     FROM customers WHERE customer_id = $1`,
    [customerId]
  );
  return result.rows[0] || null;
}

export async function createCustomer({ name, location, phoneNumber }) {
  const result = await query(
    `INSERT INTO customers (name, location, phone_number)
     VALUES ($1, $2, $3)
     RETURNING customer_id, name, location, phone_number, created_at`,
    [name.trim(), location?.trim() || null, phoneNumber?.trim() || null]
  );
  return result.rows[0];
}

export async function updateCustomer(customerId, { name, location, phoneNumber }) {
  const result = await query(
    `UPDATE customers
     SET name = $2, location = $3, phone_number = $4
     WHERE customer_id = $1
     RETURNING customer_id, name, location, phone_number, created_at`,
    [customerId, name.trim(), location?.trim() || null, phoneNumber?.trim() || null]
  );
  if (!result.rows[0]) throw new AppError('Customer not found', 404);
  return result.rows[0];
}

export async function findOrCreateCustomer({ name, location, phoneNumber, customerId, customer_id }) {
  const selectedCustomerId = customerId ?? customer_id ?? null;

  if (selectedCustomerId) {
    const existing = await getCustomerById(selectedCustomerId);
    if (!existing) throw new AppError('Customer not found', 404);
    return existing;
  }

  const trimmedName = name?.trim();
  if (trimmedName) {
    const existingByName = await query(
      `SELECT customer_id, name, location, phone_number, created_at
       FROM customers
       WHERE LOWER(name) = LOWER($1)
       ORDER BY customer_id
       LIMIT 1`,
      [trimmedName],
    );

    if (existingByName.rows[0]) {
      return existingByName.rows[0];
    }
  }

  return createCustomer({ name, location, phoneNumber });
}
