...
export async function buildSalesReportPdfBuffer(analytics, title, periodLabel, generatedBy) {
  ...
  // Header
  doc.fontSize(18).font('Helvetica-Bold').text('BigMLPG Portal', { align: 'left' });
  doc.moveDown(0.2);
...
export async function buildSalesLogPdfBuffer(rows, title, generatedBy, analytics = {}) {
  ...
  doc.fontSize(16).font('Helvetica-Bold').text('BigMLPG Portal', { align: 'left' });
  doc.moveDown(0.2);
  ...
  const availableWidth = doc.page.width - doc.page.margins.left - doc.page.margins.right;
  const rel = [0.10, 0.13, 0.15, 0.12, 0.08, 0.06, 0.08, 0.10, 0.12];
  ...
  for (const row of rows) {
    ...
    const balancePaid = row.payment_option === "Credit" ? "Credit Creation" : row.payment_option === 'Fully Paid'
      ? `Fully Paid (${Number(row.total_amount || 0).toFixed(2)})`
  ...
  if (shouldRenderSections) {
    ...
    const sectionWidth = availableWidth;
    const sectionGap = 20;
    const columnWidth = (sectionWidth - sectionGap) / 2;
    const sectionHeaderWidth = columnWidth * 0.7;
    const sectionValueWidth = columnWidth * 0.3;
    ...
    const renderSummarySection = (title, bodyRows, startY, x, width) => {
      ...
      bodyRows.forEach(([label, value]) => {
        doc.text(String(label), x, bodyY, { width: headerWidth });
        doc.text(String(value), x + headerWidth, bodyY, { width: valueWidth, align: 'right' });
        bodyY += 14;
      });
      return bodyY;
    };

    const renderMixSection = (startY, x, width) => {
      const headerWidth = width * 0.3;
      const valueWidth = width * 0.3;
      doc.font('Helvetica-Bold').fontSize(10);
      doc.text('Product & Inventory Mix', x, startY, { width });

      doc.font('Helvetica-Bold').fontSize(8);
      doc.text('Brand - Weight', x, startY + 18, { width: headerWidth });
      doc.text('Units Sold', x + headerWidth, startY + 18, { width: valueWidth, align: 'right' });
      ...
      filteredProductMix.forEach((item) => {
        ...
        doc.text(`${brandLabel} - ${weightClass.toFixed(weightClass % 1 === 0 ? 0 : 1)} kg`, x, mixY, { width: headerWidth });
        doc.text(String(unitsSold), x + headerWidth, mixY, { width: valueWidth, align: 'right' });
        mixY += 12;
      });

      doc.font('Helvetica-Bold').fontSize(8);
      doc.text('Total Units Sold', x, mixY + 4, { width: headerWidth });
      doc.text(String(totalUnitsSold), x + headerWidth, mixY + 4, { width: valueWidth, align: 'right' });
      return mixY + 24;
    };

    const renderLocationSection = (startY, x, width) => {
      const headerWidth = width * 0.3;
      const valueWidth = width * 0.3;
      doc.font('Helvetica-Bold').fontSize(10);
      doc.text('Locations Summary', x, startY, { width });

      doc.font('Helvetica-Bold').fontSize(8);
      doc.text('Location', x, startY + 18, { width: headerWidth });
      doc.text('LPG Units Sold', x + headerWidth, startY + 18, { width: valueWidth, align: 'right' });

      doc.font('Helvetica').fontSize(8);
      let locationY = startY + 32;
      locationSummaryRows.forEach((item) => {
        const unitsSold = Number(item.unitsSold ?? 0);
        doc.text(String(item.location), x, locationY, { width: headerWidth });
        doc.text(String(unitsSold), x + headerWidth, locationY, { width: valueWidth, align: 'right' });
        locationY += 12;
      });

      doc.font('Helvetica-Bold').fontSize(8);
      doc.text('Total Units Sold', x, locationY + 4, { width: headerWidth });
      doc.text(String(totalLocationUnitsSold), x + headerWidth, locationY + 4, { width: valueWidth, align: 'right' });
      return locationY + 24;
    };

    const hasSummary = Boolean(summary && Object.keys(summary).length > 0);
    const hasMix = Array.isArray(filteredProductMix) && filteredProductMix.length > 0;
    const hasLocation = Array.isArray(locationSummaryRows) && locationSummaryRows.length > 0;

    const renderLeftColumn = () => {
      if (hasSummary) {
        currentY = ensurePageSpace(160);
        currentY = renderSummarySection('Summary Totals', [
          ['Total Sales Revenue', formatPhpCurrency(Number(summary.totalSalesRevenue ?? 0))],
          ['Total Net Income', formatPhpCurrency(Number(summary.netIncomeQualified ?? summary.netIncome ?? 0))],
          ['Net Without Expenses', formatPhpCurrency(Number(summary.netIncomeQualified ?? summary.netIncome ?? 0) + Number(summary.totalExpenses ?? 0))],
          ['Fully Paid Net Income', formatPhpCurrency(Number(summary.netIncomeFullyPaid ?? summary.netIncomeWithoutCredit ?? 0))],
          ['Expected Credit Income', formatPhpCurrency(Number(summary.expectedNetIncome ?? 0))],
          ['Remaining Credit Balance', formatPhpCurrency(Number(summary.totalCreditBalance ?? 0))],
          ['Total Sales Expenses', formatPhpCurrency(Number(summary.totalExpenses ?? 0))],
        ], currentY, sectionX, columnWidth);
      }

      if (hasMix) {
        currentY = ensurePageSpace(140);
        currentY = renderMixSection(currentY, sectionX, columnWidth);
      }

      if (hasLocation) {
        currentY = ensurePageSpace(80 + locationSummaryRows.length * 12);
        currentY = renderLocationSection(currentY, sectionX, columnWidth);
      }
    };
    renderLeftColumn();
  }
